...
def showchart(request):
    ...
    queryset = Employee.objects.values('department_name').annotate(num_employee=Count('department_id')).all()
    for emp in queryset:
        labels.append(emp['department_name'])
        data.append(emp['num_employee'])
    
    context = {'title': 'Chart', 'labels': labels, 'data': data}    
